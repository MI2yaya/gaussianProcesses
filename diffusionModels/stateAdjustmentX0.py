...
def computeCovars(model, x,P, H, N=10):
    xs = []
    for _ in range(N):
        with torch.no_grad():
            x_recon = diffusion.denoise_states(
                torch.tensor(x, dtype=torch.float32, device=device).unsqueeze(0),
                clamp_min=-50, clamp_max=50,
                lam=0.5
            ).squeeze(0)
    ...
    measurement_preds = xs @ H.T
    measurement_avg = np.mean(measurement_preds, axis=0)
    R = np.zeros((H.shape[0], H.shape[0]))
    for m in measurement_preds:
        diff = (m - measurement_avg).reshape(-1,1)
        R += diff @ diff.T
    R /= (N-1)

    epsilon = xs - x  
    epsilon_avg = np.mean(epsilon, axis=0)
    Q = np.zeros((avg.shape[0], avg.shape[0]))
    for e in epsilon:
        diff = (e - epsilon_avg).reshape(-1, 1)
        Q += diff @ diff.T
    Q /= (N - 1)

    return R, Q, np.median(xs, axis=0)
...
dataset = TensorDataset(
    ...
    torch.tensor(true_states, dtype=torch.float32).reshape(-1, true_states.shape[-1])
)
...
for z in measurements:
    ...
    R,Q, x_adjusted = computeCovars(model,x_pred,P_pred,kfAdjusted.H,N=3)
    print(f"R:{R}\nQ:{Q}\nx_pred:{x_pred}\nx_adjusted:{x_adjusted},\nz:{z}")
    delta = x_adjusted - x_pred
    max_delta=10
    norm_delta = np.clip(delta, -max_delta, max_delta)
    x_adjusted = x_pred + norm_delta
    kfAdjusted.x = x_adjusted
    #kf.R = R
    #kf.Q = Q
