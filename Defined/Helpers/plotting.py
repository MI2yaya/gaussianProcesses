...
def plotMSE(xs, ys, Ms, r, q, save=False, name="smth.png"):
    fig = plt.figure(figsize=(10, 6))
    ax1 = fig.add_subplot(131)
    ax2 = fig.add_subplot(132)
    ax3 = fig.add_subplot(133)
    ax1.set_title("Scalar Random Walk")
    ax1.plot(xs, label='True States', color='blue')
    ax1.plot(ys, label='Observations', color='orange')
    ax1.plot(Ms, label='Kalman Filter Estimate', color='green')
    ax1.set_title("Scalar Random Walk")
    ax1.set_xlabel("Time Steps")
    ax1.legend()
    ax2.set_title("State MSE over Time")
    ax2.plot([mean_squared_error(xs[:i+1], Ms[:i+1]) for i in range(len(xs))], label='RMSE', color='blue')
    ax2.plot([r]*len(xs), label='Process Noise', color='red', linestyle='--')
    ax2.set_xlabel("Time Steps")
    ax2.set_ylabel("RMSE")
    ax2.legend()
    ax3.set_title("Measurement MSE over Time")
    ax3.plot([mean_squared_error(ys[:i+1], Ms[:i+1]) for i in range(len(ys))], label='RMSE', color='blue')
